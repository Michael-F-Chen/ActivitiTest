...
/**
 * Created by ChenFei on 2018/8/5.
 * 启动时初始化H2数据库数据
 */
@Component
public class H2DataInit implements CommandLineRunner {
...
//        ProcessEngineConfiguration config = ProcessEngineConfiguration.createProcessEngineConfigurationFromResourceDefault();
    ...
    private static User saveUser(IdentityService is, String id) {
    ...
    private static Group saveGroup(IdentityService is, String id, String name) {
    ...
    private static void saveRel(IdentityService is, User u, Group g) {
        is.createMembership(u.getId(), g.getId());
    }

    private void VacationDeploy(){
        ProcessEngine engine = ProcessEngines.getDefaultProcessEngine();
        // 存储服务
